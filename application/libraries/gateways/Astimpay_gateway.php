...
class Astimpay_gateway extends App_gateway
{
    ...
    public function __construct()
    {
         ...
         * Gateway name
         */
        $this->setName('AstimPay BD Payment Methods');
        ...
        $this->setSettings([
            [
                'name'      => 'api_key',
                'encrypted' => true,
                'label'     => 'AstimPay API KEY',
            ],
            [
                'name'      => 'api_url',
                'encrypted' => true,
                'label'     => 'AstimPay API URL',
            ],
            [
                'name'          => 'exchange_rate',
                'encrypted'     => true,
    ...
    public function process_payment($data)
    {
        ...
        $returnUrl = site_url('gateways/astimpay/verify_payment?invoiceid=' . $data['invoice']->id . '&hash=' . $data['invoice']->hash);
        $webhookUrl = site_url('gateways/astimpay/webhook/' . $webhookKey);
        $invoiceUrl = site_url('invoice/' . $data['invoice']->id . '/' . $data['invoice']->hash);

        if ($currency !== 'BDT') {
            $amount *= $this->decryptSetting('exchange_rate');
        }
        ...
        try {
            $astimPay = new AstimPayApi($this->decryptSetting('api_key'), $this->decryptSetting('api_url'));
    ...
    /**
     * Retrieve payment from AstimPay
     *
    ...
    public function fetch_payment($invoice_id = null)
    {
        try {
            $astimPay = new AstimPayApi($this->decryptSetting('api_key'), $this->decryptSetting('api_url'));
            if ($invoice_id) {
                return $astimPay->verifyPayment($invoice_id);
            } else {
                return $astimPay->executePayment();
            }
        } catch (\Exception $e) {
